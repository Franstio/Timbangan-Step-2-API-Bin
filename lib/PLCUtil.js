...
export const readCmd = async (address, val) => {
    ...
    try {
        ...
        console.log({openmodbus:err,status:client.isOpen});
...
export const openModbus = async () => {
    ...
    client = new ModbusRTU(new SerialPort({lock:false,path:"/dev/ttyUSB0",baudRate:9600,autoOpen:true,parity:'none',dataBits:8,stopBits:1}));
...
export const closeModbus = async () => {
    try {
        await new Promise((resolve) => {
            try {
                console.log({modbus: client.isOpen});
                client.close(() => {
                    console.log("closing");
                    client.destroy(() => {
