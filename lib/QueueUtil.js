...
// });
QueuePLC.process(1,async (job,done)=>{
    const res = job.data.type == "READ" ? await readCmd(job.data.address,job.data.value) : await writeCmd(job.data);
    if (res.success)
    { 
       LogWrite(`Data berhasil dikirim ke PLC, Data: ${JSON.stringify(job.data)}, Result: ${JSON.stringify(res.type=="READ" ? res.msg : res)}` );
       done(null,res.msg || res);
       return res;
    }
    else
    {
...
SensorObserveQueue.on('failed',(j,e)=>{
  console.log(e.message);
});
QueuePLC.on('failed',(j,e)=>{
  console.log(e.message);
});
SensorObserveQueue.on('error',(e)=>{
  console.log(e.message);
});
QueuePLC.on('error',(e)=>{
  console.log(e.message);
});
SensorObserveQueue.process(1,async (job,done)=>
{
  try
  {
    const response = [];
    console.log("START READ SENSOR");
    const job =  await QueuePLC.add({address:0,value:10,type:"READ"},{priority:5,removeOnFail:{age: 60*10,count:10},timeout:3000,removeOnComplete:{age:60,count:5}});
    const plcData = await job.finished();
    console.log(JSON.stringify(plcData));
    if (plcData == undefined || plcData == null || plcData.success == false)
    { 
      console.log(plcData?.msg ?? "Error");
      done(plcData?.msg ?? "Error",null);
      return null;
    }
    ...
    {
        await pushPayloadData({id:1,address:7,value: 1});
    }
    if ( plcData.data[greenLamp.address] == 1 && runningTransaction.isReady)
    {    
        await pushPayloadData({id:1,address:8,value: 0});    
    }
    if (runningTransaction.allowReopen)
      await observeLock(io,plcData.data);
    else  
