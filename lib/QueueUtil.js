...
//    LogWrite("Berhasil Membaca:\n"+ JSON.stringify(observationDataList));
    ...
    if ((yellowLamp != undefined ) && ( plcData.data[yellowLamp.address] != 1  && !runningTransaction.isRunning))
    {
        pushPayloadData({id:1,address:7,value: 1});
    }
    if ( (greenLamp != undefined) && (plcData.data[greenLamp.address] != 0 ) && !runningTransaction.isRunning)
    {    
        pushPayloadData({id:1,address:8,value: 0});    
    }
    LogWrite([plcData.data[yellowLamp.address],plcData.data[greenLamp.address]]);
    done(null,response);
});
