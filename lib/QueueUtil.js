...
QueuePLCConnection.process( async (job,done)=>{
    try
    {
        await client.connectRTU("COM11", { baudRate: 9600,dataBits:8,stopBits:1,parity: 'none'});
...
SensorObserveQueue.process(async (job,done)=>{
    ...
    LogWrite("Berhasil Membaca "+ JSON.stringify(response));
    done(null,response);
